...
class LR0:
            ...
            for symbol in self.grammar.get_all_symbols():
                ...
                for production in state.set:
                    if production.dot < len(production.right_hand_side) and \
                        ...
                        goto_productions.append(copy.deepcopy(production))
    ...
    def create_parsing_table(self):
        ...
        for state in states:
            ...
            shift = 0
            #print("for:" + str(state))
            for prod in state.set:
                #print("\tfor prod:" + str(prod))
                if prod.dot == len(prod.right_hand_side):
                    #print("\t \tprod dot: " + str(prod.dot) + " len: " + str(len(prod.right_hand_side)))
                    reduce = reduce + 1
                else:
                    shift = shift + 1
            ...
            if reduce > 1:
                print("REDUCE CONFLICT")
            if reduce == 1 and shift > 0:
